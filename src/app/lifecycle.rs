...
impl CimApp {
    ...
    pub(super) fn remove_media(&mut self, i: usize) {
        ...
        self.decoder.forget(removed_id); // drop its persistent reader
                                         // Drops its render thread, and with it that thread's operator instances
                                         // and (in GPU mode) its uploaded display table.
        self.renderer.forget(removed_id);
        ...
        match loaded {
            Ok(m) => {
                ...
                self.decoder.forget(id); // reopen the file for its fresh contents
                                         // Rebuild the render thread for fresh contents: its operator
                                         // instances and its display table both describe the old ones.
